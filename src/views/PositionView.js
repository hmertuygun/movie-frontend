...
const Position = () => {
  ...
  const { userData, activeExchange, orderHistoryProgressUC } =
  ...
  const ProgressBar = (
    <div
      ...
      style={{ maxWidth: '1200px', margin: '48px auto' }}
    >
      <span className="progress-label text-muted">
        Processing positions on first launch..
      </span>
      <span className="progress-percentage text-muted">{`${
        orderHistoryProgressUC !== '100.00'
          ? orderHistoryProgressUC
          : orderHistoryProgressPV
      }%`}</span>
